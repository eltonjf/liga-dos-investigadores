...
export function Criptografo() {
  return (
    <Panel title="Dicionário de Símbolos" accent="lime">
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
        {CIPHER_ALPHABET.map(({ symbol, letter }) => (
          <div
            key={symbol}
            className="flex flex-col items-center gap-1 rounded-lg border border-ink-700 bg-ink-950 py-3"
          >
            <span className="text-xl text-neon-lime">{symbol}</span>
      ...
      <div className="mt-6 rounded-lg border border-ink-700 bg-ink-950 p-4">
        ...
        <p className="mt-2 text-center text-3xl tracking-[0.3em] text-neon-lime">{CIPHER_MESSAGE}</p>
